 ...
 * 
 */
public class ContratsASouscrire extends FrontOfficeView implements  PopupSuppressionListener
{
	...
	protected void handleSupprimer(final ContratDTO c)
	{
		...
		confirmPopup.open(confirmPopup, this);		
	}
	...
	public void refresh()
	{
		...
		for (ContratDTO c : newContrats)
		{
			...
			vl2.addComponent(b);
			if(c.nbMaxSouscription > 0 && c.nbInscrits >= c.nbMaxSouscription)
				b.setEnabled(false);
	...
	{	
		...
		// Ligne 2 - Les dates de livraison
		if (nextDateLiv==null)
		{
		...
		else
		{
			...
			str = str+"La livraison du "+df.format(nextDateLivModifiable)+" peut être modifiée jusqu'au "+df.format(datLimit)+ " minuit.";
		}
