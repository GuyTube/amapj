...
import fr.amapj.view.engine.popup.swicthpopup.SwitchPopupAction;
...
public class AjouterEnMasseAdhesion extends WizardFormPopup
{
	...
	protected void configure()
	{
		...
		add(Step.SAISIE,()->addFieldSaisie(),()->writeToModel());
	}
	...
	protected void addFieldSaisie()
	{
		if(lines.size()==0)
		{
			addLabel("Il n'y a pas de cotisation à créer", ContentMode.HTML);
			setWidth(60);
			return;
		}
	...
	{
		SwitchPopupAction popup = new SwitchPopupAction("Actions possibles",60);
		
		popup.setLine1("Veuillez indiquer ce que vous souhaitez faire :");
		popup.addLine("Positionner toute la colonne Montant avec la valeur "+new CurrencyTextFieldConverter().convertToString(periode.montantConseille)+" €",()->colonneMontant());
		popup.addSeparator();
		popup.addLine("Positionner toute la colonne Etat du paiement avec la valeur A FOURNIR",()->colonneEtatPaiement(EtatPaiementAdhesion.A_FOURNIR));
		popup.addLine("Positionner toute la colonne Etat du paiement avec la valeur ENCAISSE",()->colonneEtatPaiement(EtatPaiementAdhesion.ENCAISSE));
		popup.addSeparator();
		popup.addLine("Positionner toute la colonne Type du paiement avec la valeur CHEQUE",()->colonneTypePaiement(TypePaiementAdhesion.CHEQUE));
		popup.addLine("Positionner toute la colonne Type du paiement avec la valeur ESPECES",()->colonneTypePaiement(TypePaiementAdhesion.ESPECES));
		popup.addLine("Positionner toute la colonne Type du paiement avec la valeur VIREMENT",()->colonneTypePaiement(TypePaiementAdhesion.VIREMENT));
		popup.addLine("Positionner toute la colonne Type du paiement avec la valeur INTERNET",()->colonneTypePaiement(TypePaiementAdhesion.INTERNET));
