 ...
 * 
 * Outil utilisé par les feuilles d'émargement en début de saison
 *
 */
public class EGGrilleEmargementTool 
{
	public EGGrilleEmargementTool()
	{		
	}
			...
			int nbColGauche, Map<Utilisateur, ContratDTO> contrats)
	{
		...
		// Calcul du nombre de colonnes :  nb produit
		int nbColProd = prods.size();
		
		int nbColTotal = nbColGauche+nbColProd;
		...
		for (int i = 0; i < utilisateurs.size(); i++)
		{
			...
			contructRow(et,contratDto,utilisateur,nbColProd,nbColGauche,dates);
		}	
	}
	...
	private void contructRow(ExcelGeneratorTool et, ContratDTO contratDto, Utilisateur utilisateur,int nbColDateProd,int nbColGauche, List<ModeleContratDate> dates)
	{
		...
		{	
			...
			for (int k = 0; k < dates.size(); k++)
			{
				...
				if(contratDto.qte[i][j]>max) {
					max = contratDto.qte[i][j];
				}
			}
			CellStyle style = et.nonGrasCentreBordure;
			
			et.setCellQte(index, max, style);
			index++;
		}
