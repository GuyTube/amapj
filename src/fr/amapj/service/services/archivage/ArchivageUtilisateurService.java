...
public class ArchivageUtilisateurService
{
	...
	public ArchivableState computeArchivageState(Long idUtilisateur,ParametresArchivageDTO param)
	{
		...
		TypedQuery<Paiement> q = em.createQuery("select p from Paiement p WHERE p.contrat.utilisateur.id=:id AND p.etat!=:etat",Paiement.class);
		...
		Query q2 = em.createQuery("select count(distinct(c.modeleContratDate)) from ContratCell c WHERE c.modeleContratDate.dateLiv>=:d AND c.contrat.utilisateur.id=:id");
		q2.setParameter("d", ref);
		q2.setParameter("id", idUtilisateur);

		int nbLiv = SQLUtils.toInt(q2.getSingleResult());
		if (nbLiv != 0)
		{
		...
		q2 = em.createQuery("select count(c) from PeriodeCotisationUtilisateur c WHERE c.periodeCotisation.dateFin>=:d AND c.utilisateur.id=:id");
		q2.setParameter("d", ref);
		q2.setParameter("id", idUtilisateur);

		if (SQLUtils.toInt(q2.getSingleResult()) != 0)
		{
	...
	private String checkPermanenceFuture(EntityManager em,Long idUtilisateur) 
	{
		TypedQuery<Date> q = em.createQuery("select max(d.periodePermanenceDate.datePerm) from PermanenceCell d WHERE d.periodePermanenceUtilisateur.utilisateur.id=:id",Date.class);
		q.setParameter("id",idUtilisateur);
		Date dernierePermance = q.getSingleResult();
	...
	private String checkDateCreation(EntityManager em, Utilisateur u,int delai) 
	{
